...
$hostname = "mysql1.mydevil.net";
$database ="m13730_wallet";
...
function get_savings_outcome_by_category($logged_user) {
    $query = "
        SELECT
            c.name category_name, c.id category_id, sum(o.amount) amount
        FROM 
            savings_operations o join categories c on o.category_id = c.id
        WHERE
            o.us_id = ".$logged_user."
...
function savings_operations($logged_user) {
    $query = "SELECT * FROM savings_operations WHERE us_id = ".$logged_user." ORDER BY when_created DESC;";
    $result = mysql_query($query);
    $a = Array();
    while($row = mysql_fetch_array($result)) { 
        array_push($a, $row);
...
function delete_savings_operation($operation_id, $logged_user) {
    mysql_query("DELETE FROM savings_operations WHERE id = ".$operation_id);
    recalculate_savings($logged_user);
}
...
function get_spent_savings($logged_user) {
    $result = mysql_query("SELECT * FROM savings WHERE us_id = ".$logged_user);
    while($row = mysql_fetch_array($result)) { 
        return $row['amount_out'];
    }
    return 0;
}
...
function increase_decrease_in_savings($logged_user, $outcome) {
    $sql = "UPDATE savings SET amount_out = amount_out + ".$outcome." WHERE us_id = ".$logged_user;
    mysql_query($sql);
}

function save_savings_operation($logged_user, $proper_amount_outcome, $outcome_date, $outcome_category) {
    is_numeric($proper_amount_outcome) ? $proper_amount_outcome = $proper_amount_outcome : $proper_amount_outcome = 0;
    increase_decrease_in_savings($logged_user, $proper_amount_outcome);
    
    if ($proper_amount_outcome > 0) {
        $sql = " INSERT INTO savings_operations SET
                 us_id = ".$logged_user.",
